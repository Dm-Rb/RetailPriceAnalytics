...
from parsers.edostavka_by.cache import Cache
...
class CategoryService(Cache):
    def __init__(self, session_factory):
        ...
        with session_factory() as session:
            ...
            for item in catalog_db.get_all_product_display_by_source(source):
                self.product_article.setdefault(item.article, item.product_id)
    ...
    def get_product_id(self, manufacturer_id, name, description, composition, storage_info, unit):
    ...
    def get_manufactory_id(self, trademark, full_name, country):
        full_name_hash = self.string_hash(full_name)
        if self.manufacturers.get(full_name_hash, None):
            return self.manufacturers[full_name_hash]

        with self.__session_factory() as session:
            catalog_db = CatalogCRUD(session)
            manufactory_id = catalog_db.save_new_manufactory(trademark=trademark, full_name=full_name, country=country)
        self.manufacturers.setdefault(full_name_hash, manufactory_id)
    ...
    def get_product_display_id(self, product_id: int, article: str, source: str):
        if self.product_article.get(article, None):
            return self.product_article[str(article)]
    ...
    def save_product_article_relations(self, source, product_id, article):
        with self.__session_factory() as session:
            catalog_db = CatalogCRUD(session)

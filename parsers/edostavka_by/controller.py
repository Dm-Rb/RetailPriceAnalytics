...
from parsers.edostavka_by.service import CategoryService
...
def main():
    ...
    for product in spider.crawl():
        if service_data.product_article.get(str(product.productId), None):
            product_id = service_data.product_article[str(product.productId)]
